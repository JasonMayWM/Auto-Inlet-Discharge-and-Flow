...
// === Low Flow Reset Variables ===
bool enableLowFlowReset = false; // Disabled by default
const float LOW_FLOW_THRESHOLD = 0.1; // L/min
const unsigned long LOW_FLOW_DURATION_THRESHOLD = 5000; // 5 seconds
unsigned long lowFlowDetectTime = 0;
bool lowFlowTriggeredActive = false; // Flag to indicate the low flow reset condition has been met and action is pending
...
float pressureToleranceInlet = 0.02f; 
...
struct CircularBuffer {
  ...
  void push(int value) {
    ...
    else { start = (start + 1) % arraySize; }
  }
  float average() {
    if (count == 0) return 0;
    float sum = 0;
    for (int i = 0; i < count; i++) { sum += buffer[(start + i) % arraySize]; }
    return sum / count;
  }
};
...
public:
    int _stepPin, _dirPin, _enaPin;
    long _maxSteps, _initialInterval, _runInterval, _accelerationSteps;
    ...
    String _name;                 
    long _stepCount, _stepCountAcc;
    bool _previousDirection;
    unsigned long _motorLastStepTime;
    long _currentInterval; 
    float _slowdownPressureThreshold1, _slowdownFactor1;
    float _slowdownPressureThreshold2, _slowdownFactor2;   
    ...
    MotorController(int stepPin, int dirPin, int enaPin,
        ...
        _stepCount = 0; _stepCountAcc = 0; _previousDirection = true; 
        _motorLastStepTime = 0; _currentInterval = _initialInterval; 
        _pressureControlStartTime = 0; _timedOut = false;             
        pinMode(_stepPin, OUTPUT); pinMode(_dirPin, OUTPUT); pinMode(_enaPin, OUTPUT);
        digitalWrite(_stepPin, LOW); digitalWrite(_enaPin, HIGH); 
    }
    ...
    void calculateInterval() {
        if (_isRecoveringOvershoot) { 
            ...
            if (_currentInterval == 0) _currentInterval = 1; 
            return; 
        }
        ...
        if (_stepCountAcc < _accelerationSteps) {
            ...
            _stepCountAcc++;
        } else { _currentInterval = _runInterval; }
    ...
    void managePressure(bool targetPressureFlag, bool globalResetFlag) {
        ...
        if (!globalResetFlag && isControlEnabled() && _setPressureRef != 0 && !targetPressureFlag) {
            if (!_isRecoveringOvershoot) {
                ...
                if (overshootDetected) {
                    ...
                    digitalWrite(_dirPin, reverseDirForPin); 
        ...
        if (!isControlEnabled() || targetPressureFlag || _isRecoveringOvershoot) { 
            return;
        }
...
MotorController inletMotor(
    ...
    currentPressureInlet, setPressureInlet, controlEnableInlet, notifyInlet, 
    maxPressureInlet, 
...
MotorController dischargeMotor(
    ...
    currentPressureDischarge, setPressureDischarge, controlEnableDischarge, notifyDischarge, 
    minimumPressureDischarge, 
    0.2f, 4.0f, 0.0f, 1.0f, "Discharge", 
    reverseStepsDischarge, slowFactorDischarge, pressureToleranceDischarge
);

void pressureSensorCore1(); // Forward declaration

// New function for low flow check
void checkLowFlowReset() {
  if (!enableLowFlowReset || reset || lowFlowTriggeredActive) { 
    return;
  }
  if (currentFlow < LOW_FLOW_THRESHOLD) {
    if (lowFlowDetectTime == 0) { 
      lowFlowDetectTime = millis();
    } else { 
      if (millis() - lowFlowDetectTime > LOW_FLOW_DURATION_THRESHOLD) {
        lowFlowTriggeredActive = true; 
        lowFlowDetectTime = 0; 
      }
    }
  } else { 
...
void loop() {
  serial();
  checkLowFlowReset(); // Call the new function

  if (lowFlowTriggeredActive) {
    Serial.println("ALERT: Low flow rate detected for the specified duration! Initiating system reset.");
    setPressureInlet = 0.0f;
    setPressureDischarge = 0.0f;
    
    inletMotor.resetTimeout(0.0f);      
    inletMotor.setNotifyFlag(false);    
    notifyInlet = false;                

    dischargeMotor.resetTimeout(0.0f);
    dischargeMotor.setNotifyFlag(false);
    ...
    reset = true; 
    lowFlowTriggeredActive = false; 
  }

  // Inlet Pressure Achievement Logic
  ...
  // Discharge Pressure Achievement Logic
...
const char *Help_Message[] = {
  ...
  "enable_low_flow_reset  - Enable auto motor reset on low flow",
  "disable_low_flow_reset - Disable auto motor reset on low flow",
...
void processCommand() {
  ...
  } else if (serialStr.equalsIgnoreCase("enable_low_flow_reset")) {
    enableLowFlowReset = true;
    lowFlowDetectTime = 0; 
    lowFlowTriggeredActive = false; 
    Serial.println("Low flow rate reset ENABLED.");
  } else if (serialStr.equalsIgnoreCase("disable_low_flow_reset")) {
    enableLowFlowReset = false;
    lowFlowDetectTime = 0; 
    lowFlowTriggeredActive = false;
    Serial.println("Low flow rate reset DISABLED.");
...
// Orphaned functions, logic moved/integrated and functions removed.
